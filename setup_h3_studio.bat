...
:find_python
set "SYSTEM_PYTHON="
rem Only exit code zero means success. PyManager errors can be negative in cmd.
where py >nul 2>&1
if not errorlevel 1 (
  py -3.12 -c "import sys; assert sys.version_info[:2] == (3, 12)" >nul 2>&1
  if errorlevel 0 if not errorlevel 1 set "SYSTEM_PYTHON=py -3.12"
  if not defined SYSTEM_PYTHON (
    py -3.11 -c "import sys; assert sys.version_info[:2] == (3, 11)" >nul 2>&1
    if errorlevel 0 if not errorlevel 1 set "SYSTEM_PYTHON=py -3.11"
  )
)
if not defined SYSTEM_PYTHON (
  ...
  if not errorlevel 1 (
    ...
    if errorlevel 0 if not errorlevel 1 set "SYSTEM_PYTHON=python"
  )
)
exit /b 0
...
echo If you have Python Install Manager, run: py install 3.12
echo Then verify it with: py -3.12 --version
