...
- 工具列內建 Qwen3-TTS 語音工作室：內建聲線、文字設計新聲線、參考音訊聲線複製、中文與多語台詞、拖曳音訊、GPU 排隊、歷史試聽與 WAV 下載

### Qwen3-TTS 本機語音工作室

點工具列的「語音」即可使用。第一次使用某一模式時按「安裝目前語音模型」，工具會在 `H3Studio/data/voice_runtime/` 建立獨立 Python 環境，並將模型放在 `H3Studio/data/voice_models/`；不會修改 ComfyUI 的套件或模型。三種模式分開安裝，因此可以只下載實際需要的權重：

- **內建聲線**：0.6B CustomVoice，九種官方聲線，適合旁白與快速預覽。
- **設計新聲線**：1.7B VoiceDesign，以年齡感、音色、情緒、節奏與人物氣質描述創造聲線。
- **複製參考聲線**：0.6B Base，上傳短音訊並填入逐字稿後產生新台詞；無逐字稿時可改用僅擷取音色，但品質可能降低。

語音、Music 3 與影片共用 H3 Studio 的 GPU 鎖，不會同時搶顯存。語音結果、參考音訊與工作歷史都放在被 Git 排除的 `H3Studio/data/`，不會推送給同事。聲線複製僅可使用本人聲音，或已取得本人同意／合法授權的素材。

目前 Qwen3-TTS 是由每一台執行 H3 Studio 的電腦本機運算，不會經由 ComfyUI Gateway 送到管理主機；只連遠端 ComfyUI、且本機沒有 NVIDIA GPU 的同事仍可使用影片功能，但無法在自己的工作站執行本機語音模型。
